...
class EvaluationManager:
    def __init__(self, metric_calculators: List[MetricCalculator], matcher_type: str):
        self.metric_calculators = metric_calculators
        if matcher_type.lower() == 'antique':
            self.matcher = AntiqueMatcher()
        elif matcher_type.lower() == 'wikipedia':
            self.matcher = WikipediaMatcher()
        else:
    ...
    def evaluate(self, queries: Dict[str, str], qrels: Dict[str, Dict[str, int]], k: Optional[int] = None) -> Dict[
        ...
        for query_id, query_text in queries.items():
            retrieved_docs = self.matcher.match(query_text, k)  # Match top k elements
            print(len(retrieved_docs))
            metrics_results = {}
            for metric_calculator in self.metric_calculators:
                metric_name = metric_calculator.__class__.__name__
                if metric_name in ["ReciprocalRankCalculator", "RecallCalculator", "PrecisionCalculator"]:
